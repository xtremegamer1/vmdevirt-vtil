...
const vtil::routine* devirtualize
  (const std::uintptr_t module_base, const std::uintptr_t image_base, const std::uint32_t image_size, 
   const std::uintptr_t vm_entry_rva, const fs::path& bin_name, const fs::path& routines_folder)
{
    ...
    std::ofstream virtual_assembly(routines_folder / 
...
int __cdecl main(int argc, const char *argv[])
{
  ...
  fs::create_directory(routines_folder);
  ...
  for (const auto& vm_entry_rva : vm_entry_rvas)
  {
    ...
    std::printf("\n[%.4d]  STARTING FROM NEW VMENTER AT %p (%p)\n\n", vmenter_number++,
                                                              vm_entry_rva + module_base,
                                                              vm_entry_rva + image_base);
    std::stringstream hex_rva;
    hex_rva << std::hex << vm_entry_rva;
    ...
    catch(const std::exception& e)
    {
      ...
      {
        prem = devirtualize(module_base, image_base, image_size, vm_entry_rva, bin_name, routines_folder);
        if (!prem)
        {
